...
final class SessionController {
    ...
    func handleBreakEnvironmentFailure() {
        ...
        case .running(let session) where session.mode.isBreak:
            ...
            cleanupBreakEnvironment(animated: false)
            lastError = "Pawse could not keep every display covered. The break was canceled for safety."
            state = .idle(selectedMode: .focus)
            nowSnapshot = now
        case .breakEntering:
            cleanupBreakEnvironment(animated: false)
    ...
    private func handleOverlayFailure() {
        cleanupBreakEnvironment(animated: false)
        lastError = "Pawse could not cover every display. The break was canceled for safety."
        state = .idle(selectedMode: .focus)
        refreshNow()
    }
